...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的家教订单 </p>
    </div>
    <!-- 标题 -->
                ...
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.jiajiaoName}}</td>
                            <td style="width: 80px;">{{item.sexValue}}</td>
                            <td style="width: 80px;">{{item.xuqiuValue}}</td>
                            <td style="width: 60px;">{{item.jiajiaoIdNumber}}</td>
                            <td style="width: 60px;">{{item.jiajiaoPhone}}</td>
                            <td style="width: 60px;">{{item.jiajiaoEmail}}</td>
                            <td style="width: 250px;">
                                <img :src="item.jiajiaoPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 150px;">{{item.jiajiaoTime}}</td>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 jiajiaoOrderContent 字段去除无用字符 如果字段长度大于60  就截取到60
            jiajiaoOrderContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            //列表查询
            ,pageList(jiajiaoOrderTypes) {
                this.jiajiaoOrderTypes = jiajiaoOrderTypes;
                // 获取列表数据
                layui.http.request('jiajiaoOrder/list', 'get', {
                    page: 1,
                    limit: this.limit,
                    jiajiaoOrderTypes: vue.jiajiaoOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('jiajiaoOrder/list', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    jiajiaoOrderTypes: this.jiajiaoOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                // 获取商品详情信息
                layui.http.requestJson(`jiajiaoOrder/pay`, 'PPST',{
                    id:id
                }, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
            ...
            ,refund(id) {
                // 获取商品详情信息
                layui.http.request(`jiajiaoOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        // 获取列表数据
        http.request('jiajiaoOrder/list', 'get',{
            page: 1,
            limit: limit,
            jiajiaoOrderTypes: vue.jiajiaoOrderTypes
        }, function(res) {
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('jiajiaoOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            jiajiaoOrderType: vue.jiajiaoOrderType
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
