            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">家教收藏展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">家教姓名</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="jiajiaoName" id="jiajiaoName" placeholder="家教姓名" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../jiajiao/detail.jsp?id='+item.jiajiaoId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.jiajiaoPhoto?item.jiajiaoPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../jiajiao/detail.jsp?id='+item.jiajiaoId)"><span>{{item.jiajiaoName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.jiajiaoNewMoney"><span style="color: red;">{{item.jiajiaoNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteJiajiaoCollection(item.id)">删除 {{item.jiajiaoName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 jiajiaoCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            jiajiaoCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteJiajiaoCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('jiajiaoCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#jiajiaoName').val() != null && jquery('#jiajiaoName').val() != "" && jquery('#jiajiaoName').val() != "null" ) {
                param['jiajiaoName'] = jquery('#jiajiaoName').val();
            }
            ...
            // 当前表
                                                    // 获取列表数据
            http.request('jiajiaoCollection/list', 'get', param, function(res) {
