            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">家教需求展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">家教需求</label>
                          <div class="layui-input-inline">
                              <input type="text" name="xuqiuName" id="xuqiuName" placeholder="家教需求" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="yonghuIdStart" id="yonghuIdStart"  lay-skin="integer"   placeholder="发布用户开始" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <div class="layui-input-inline" style="margin-top: -29px">
                              <input type="text" name="yonghuIdEnd"   id="yonghuIdEnd"    lay-skin="integer|yonghuIdEnd"   placeholder="发布用户结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">家教类型</label>
                          <div class="layui-input-inline">
                              <select name="xuqiuTypes" id="xuqiuTypes" lay-filter="xuqiuTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in xuqiuTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                        ...
                        <div class="article-list">
                            <ul class="after">
                                <li v-for="(item,index) in dataList" v-bind:key="index">
                                    <a target="_self" :href="'../xuqiu/detail.jsp?id='+item.id">
                                        <img width="180" class="fl" :src="item.xuqiuPhoto" />
                                        <h5>{{item.xuqiuName}}</h5>
                                        <p class="time-sour">
                                            <span>发布时间：{{item.insertTime}} </span>
                                        </p>
                                        <p class="reamk">{{item.xuqiuContent|xuqiuContent}}...</p>
                                    </a>
                                </li>
                            </ul>
                            <div class="pages">
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 xuqiuContent 字段去除无用字符 如果字段长度大于60  就截取到60
            xuqiuContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        form.verify({
            ...
            ,yonghuIdEnd: function(value, item){ //value：表单的值、item：表单的DOM对象
                    var yonghuIdStart = $("#yonghuIdStart").val();
                 if(yonghuIdStart != null && yonghuIdStart != "" && yonghuIdStart != "null"){
                    var number = parseInt(value)- parseInt(yonghuIdStart);
                    if(number<0){
                        return '发布用户的结束值不能小于开始值';
                    }
                 }
            }
        ...
        function pageList() {
            ...
            if (jquery('#xuqiuName').val() != null && jquery('#xuqiuName').val() != "" && jquery('#xuqiuName').val() != "null" ) {
                param['xuqiuName'] = jquery('#xuqiuName').val();
            }
         
            if (jquery('#yonghuIdStart').val() != null && jquery('#yonghuIdStart').val() != "" && jquery('#yonghuIdStart').val() != "null" ) {
                param['yonghuIdStart'] = jquery('#yonghuIdStart').val();
            }
            if (jquery('#yonghuIdEnd').val() != null && jquery('#yonghuIdEnd').val() != "" && jquery('#yonghuIdEnd').val() != "null" ) {
                param['yonghuIdEnd'] = jquery('#yonghuIdEnd').val();
            }
         
            var xuqiuTypes = $("#xuqiuTypes").find("option:selected").val();
            if (xuqiuTypes != null && xuqiuTypes != "" && xuqiuTypes != "null") {
                param['xuqiuTypes'] = xuqiuTypes;
            }
                                            // 获取列表数据
            http.request('xuqiu/list', 'get', param, function(res) {
    ...
    // 当前表的字典表
       ...
       function xuqiuTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xuqiu_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.xuqiuTypesList = res.data.list;
               }
           });
       }
